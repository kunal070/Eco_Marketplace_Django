...
class ForgotPasswordPhoneView(View):
    template_name = 'auth/forgot_password_phone.html'
    ...
    def post(self, request):
        ...
        try:
            ...
            profile.phone_verification_code = otp
            profile.save()
            send_verification_code(phone_number, otp)  # Twilio SMS
            request.session['reset_phone'] = phone_number
            messages.success(request, "OTP sent to your phone number.")
            return redirect('authentication:verify_otp_reset')
        except UserProfile.DoesNotExist:
            messages.error(request, "Phone number not found.")
            return render(request, self.template_name)


class VerifyOtpAndResetPasswordView(View):
    template_name = 'auth/verify_otp_reset.html'
    ...
    def post(self, request):
        ...
        if not phone_number:
            ...
            return redirect('authentication:forgot_password_phone')
